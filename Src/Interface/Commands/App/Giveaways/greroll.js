	...
	run: async (ctx) => {
		...
		if (!ctx.interaction.member?.permissions.has(PermissionsBitField.Flags.ManageGuild) && !(owner_data && owner_data.additional_owners.includes(ctx.interaction.member?.id)) && !ctx.interaction.member?.roles.cache.has(giveawayRole?.id)) {
			return ctx.interaction.reply({
				content: `${process.env.FAILURE_EMOJI} | You need the \`Manage Server\` permissions or Giveaways role to reroll a giveaway!`,
				ephemeral: true,
			});
		}
		...
		ctx.client.giveawaysManager
			...
			.then(() => {
				ctx.interaction.reply({
					content: `${process.env.SUCCESS_EMOJI} | Giveaway rerolled!`,
					ephemeral: true,
				});
			}).catch(() => {
				ctx.interaction.reply({
					content: `${process.env.FAILURE_EMOJI} | No valid participations, no new winner could be chosen!`,
					ephemeral: true,
				});
			});
	},
