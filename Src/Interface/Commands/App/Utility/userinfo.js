...
module.exports = new App({
	...
	options: [{
		name: 'user',
		description: 'The user to get the userinfo of!',
		type: 6,
		required: false,
	}],
	...
	run: async (ctx) => {
		...
		if (data && ctx.interaction.guild.members.cache.get(user?.id) === undefined) {
		...
		else if (!data && ctx.interaction.guild.members.cache.get(user?.id) === undefined) {
			await ctx.interaction.reply({
				embeds: [{
					author: {
						name: user?.globalName || user?.username,
			...
			if (ctx.interaction.guild.members.cache.get(user?.id).presence?.activities?.length === undefined) {
				userStatus = 'No Activities Available';
			}
			else if (ctx.interaction.guild.members.cache.get(user?.id).presence?.activities?.length === 0) {
				userStatus = 'No Activities Available';
			}
			else if (ctx.interaction.guild.members.cache.get(user?.id).presence?.activities?.length > 3) {
				userStatus = 'The user has more than 3 activities.';
			}
			else {
				const array = [];
				const activities = ctx.interaction.guild.members.cache.get(user?.id).presence?.activities;
				...
				if (i.isButton()) {
					switch (i.customId) {
						...
						case two:
							...
							if (
								ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.BanMembers) ||
                                ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.ManageMessages) ||
                                ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.KickMembers) ||
                                ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.ManageRoles) ||
                                ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.ModerateMembers)
							) {
								acc = 'Server Moderator';
							}
							if (ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.ManageEvents)) {
								acc = 'Event Manager';
							}
							if (ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.ManageGuild)) {
								acc = 'Server Manager';
							}
							if (ctx.interaction.guild.members.cache.get(user?.id).permissions.has(PermissionsBitField.Flags.Administrator)) {
								acc = 'Administrator';
							}
							...
							i.editReply({
								embeds: [{
									...
									fields: [{
										name: `__Information in ${ctx.interaction.guild.name}__`,
										value: `**Joined:** <t:${Math.floor(ctx.interaction.guild.members.cache.get(user?.id).joinedAt / 1000)}:F> [<t:${Math.floor(ctx.interaction.guild.members.cache.get(user?.id).joinedAt / 1000)}:R>]\n**Nickname:** ${ctx.interaction.guild.members.cache.get(user?.id).nickname || 'No Nickname'}\n**Booster:** ${ctx.interaction.guild.members.cache.get(user?.id).premiumSince ? `${process.env.SUCCESS_EMOJI}` : `${process.env.FAILURE_EMOJI}`}\n**Boosting Since:** ${ctx.interaction.guild.members.cache.get(user?.id).premiumSinceTimestamp === null ? 'Not Boosting' : `<t:${~~(ctx.interaction.guild.members.cache.get(user?.id).premiumSinceTimestamp / 1000)}:R>`}\n**Acknowledgements:** ${acc}`,
										inline: false,
									}],
									color: 16705372,
									thumbnail: {
										url: ctx.interaction.guild.members.cache.get(user?.id).displayAvatarURL({ size: 2048 }) || user?.displayAvatarURL({ size: 2048 }),
									},
								}],
								components: [{
									type: 1,
						...
						case three:
							...
							if (ctx.interaction.guild.members.cache.get(user?.id).roles.icon) {
								iconURL = ctx.interaction.guild.members.cache.get(user?.id).roles.icon.iconURL();
							}

							let color = 'No Color';

							if (ctx.interaction.guild.members.cache.get(user?.id).roles.color) {
								color = `${ctx.interaction.guild.members.cache.get(user?.id).roles.color.hexColor} (${ctx.interaction.guild.members.cache.get(user?.id).roles.color.color})`;
							}
							...
							i.editReply({
								embeds: [{
									...
									fields: [{
										name: '__Role info__',
										value: `\n**Highest Role:** <@&${ctx.interaction.guild.members.cache.get(user?.id).roles.highest.id}>\n**Roles:** ${ctx.interaction.guild.members.cache.get(user?.id).roles.cache.sort((a, b) => b.position - a.position).map(r => r.toString()).slice(0, -1).length > 10 ? 'Too many roles to show here' : `${ctx.interaction.guild.members.cache.get(user?.id).roles.cache.sort((a, b) => b.position - a.position).map(r => r.toString()).slice(0, -1).length ? ctx.interaction.guild.members.cache.get(user?.id).roles.cache.sort((a, b) => b.position - a.position).map(r => r.toString()).slice(0, -1).join(', ') : 'The user don\'t have any roles'}`}\n**Color:** ${color}`,
										inline: false,
									}],
									color: 16705372,
									thumbnail: {
						...
						case four:
							...
							const perms = ctx.interaction.guild.members.cache.get(user?.id).permissions.toArray();
