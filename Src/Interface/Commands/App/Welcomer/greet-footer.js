...
module.exports = new App({
	...
	options: [
		...
		{
			name: 'icon',
			description: 'The image for the footer',
			type: 1,
		},
	],
	/**
	...
	run: async (ctx) => {
		...
		await Schema.findOne({
			...
			if (data) {
				if (data.embed === true) {
					if (ctx.interaction.options.getSubcommand() === 'text') {
						...
						if (message.first().content.toLowerCase() === 'cancel') {
							ctx.interaction.channel?.send({ content: 'Cancelled the setup. Run the command again to set it up.' });
							return;
						}

						if (message.first().content.toLowerCase() === 'remove') {
							ctx.interaction.channel?.send({ content: `${process.env.SUCCESS_EMOJI} | Sucessfully removed the content.` });
						...
						if (data_message.length > 500) return ctx.interaction.channel?.send({ content: `${process.env.FAILURE_EMOJI} | The message length must be less than 500 characters.` });
						data.footer_text = message.first().content;
						await data.save();
						ctx.interaction.channel?.send({ content: `${process.env.SUCCESS_EMOJI} | Sucessfully set the footer text for the welcomer.` });
					}
						...
						if (message.first().content.toLowerCase() === 'remove') {
							ctx.interaction.channel?.send({ content: `${process.env.SUCCESS_EMOJI} | Sucessfully removed the footer icon.` });
							data.footer_icon_url = null;
							await data.save();
							return;
						}

						if (!message.first().content.startsWith('https://') && message.first().content.toLowerCase() !== '<<server.icon>>' && message.first().content.toLowerCase() !== '<<member_avatar>>') return ctx.interaction.channel?.send({ content: `${process.env.FAILURE_EMOJI} | Your link doesn't start with https! Run the command again to set it up.` });

						const member = ctx.interaction.member;
						let footer_icon_url = message.first().content.replace(/<<server.icon>>/g, member?.guild.iconURL({ size: 4096 })).replace(/<<member_avatar>>/g, member?.user?.displayAvatarURL({ size: 4096 }));
						...
						const embed = new EmbedBuilder()
							.setDescription('­')
							.setFooter({ text: 'This is a preview of the footer icon.', iconURL: footer_icon_url });

						ctx.interaction.channel?.send({ embeds: [embed] }).catch(() => {
							return ctx.interaction.channel?.send({ content: `${process.env.FAILURE_EMOJI} | Your link is invalid! Run the command again to set it up.` });
						});

						data.footer_icon_url = message.first().content.split(' ')[0];
						await data.save();
						ctx.interaction.channel?.send({ content: `${process.env.SUCCESS_EMOJI} | Sucessfully set the footer icon for the welcomer. You can see the preview in the embed above.` });
					}
				}
				else {
					ctx.interaction.reply({
