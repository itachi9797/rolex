	...
	run: async (ctx) => {
		...
		if (!ctx.message.guild.members.cache.get(user?.id)) {
			return ctx.message.reply({
				content: `${process.env.FAILURE_EMOJI} | The member was not found in the guild!`,
			}).then(message => {
				setTimeout(() => {
					message.delete();
				}, 15000);
		...
		if (user?.id === ctx.message.author?.id) {
			...
			if (!ctx.message.member?.permissions.has(PermissionsBitField.Flags.KickMembers) && !(owner_data && owner_data.additional_owners.includes(ctx.message.member?.id))) {
				return ctx.message.reply({
			...
			if (!ctx.message.guild.members.me.permissions.has(PermissionsBitField.Flags.KickMembers)) {
				return ctx.message.reply({
			...
			await ctx.message.guild.members.kick(user?.id, {
			...
			ctx.message.reply({
				...
				await user?.send({
					content: `You were kicked from ${ctx.message.guild.name}!\nReason: ${reason}`,
				}).catch(() => {
					return msg.edit({
