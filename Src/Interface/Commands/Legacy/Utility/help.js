...
module.exports = new App({
     ...
     */
	run: async (ctx) => {
		const command_name = ctx.message.content.split(' ').slice(1).join(' ');
		...
		if (!command_name) {
			...
			const msg = await ctx.message.reply({
				...
				contents: `Hey ${ctx.client.owners.includes(ctx.message.author?.id) ? 'Developer' : `<@${ctx.message.author.id}>`}, here is the list of commands for you to choose from!`,
				embeds: home_embed,
				components: [
					{
						type: 1,
